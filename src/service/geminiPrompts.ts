...
export async function getPointOfView(subject, time, amount, age, gender, place): Promise<string> {
    const prompt = `אתה מדריך צופים המעביר פעילות לחניכיו בתנועת הנוער ואני רוצה שתיצור פעילות בשם 'פעילות נקודת מבט' על הנושא ${subject} שתימשך ${time}, מספר הילדים בפעילות הוא ${amount}, כיתתם היא ${age} ומינם הוא ${gender}. מקום הפעילות יהיה ב${place} .(וודא שאין שגיאות כתיב כלל) בתחילת התשובה כתוב את שם הפעילות ואת משך הזמן שלה. בכל חלק של הפעילות, ציין את הזמן הנדרש בדקות. וודא כיסוי מלא של כל ההיבטים והוראות מפורטות לפעילות. תחזיר את התשובה בפורמט של markdown style  (תוודא שאתה לא מוסיף שפות אחרות) הבט לשקף תכנים ישירים כמו שמות וזמנים ללא שינוי, אך העשר את התיאור בפרטים רלוונטיים נוספים להבנה טובה יותר. אל תכלול מצגת או וידאו בפעילות. התשובה צריכה להיות מפורטת ובעברית בלבד.`;
...
//     constructor() {
//         this.genAI = new GoogleGenerativeAI(process.env.REACT_APP_API_KEY_2);
//     }
...
//     public async getPointOfView(subject, time, amount, age, gender, place): Promise<string> {
//         const prompt = `אתה מדריך צופים המעביר פעילות לחניכיו בתנועת הנוער ואני רוצה שתיצור פעילות בשם 'פעילות נקודת מבט' על הנושא ${subject} שתימשך ${time}, מספר הילדים בפעילות הוא ${amount}, כיתתם היא ${age} ומינם הוא ${gender}. מקום הפעילות יהיה ב${place} .(וודא שאין שגיאות כתיב כלל) בתחילת התשובה כתוב את שם הפעילות ואת משך הזמן שלה. בכל חלק של הפעילות, ציין את הזמן הנדרש בדקות. וודא כיסוי מלא של כל ההיבטים והוראות מפורטות לפעילות. הבט לשקף תכנים ישירים כמו שמות וזמנים ללא שינוי, אך העשר את התיאור בפרטים רלוונטיים נוספים להבנה טובה יותר. אל תכלול מצגת או וידאו בפעילות. התשובה צריכה להיות מפורטת ובעברית בלבד.`;
//         return await this.generateContent(prompt);
//     }

//     public async getContentActivity(subject, time, amount, age, gender, place): Promise<string> {
...
//     public async getScoutingTime(subject, time, amount, age, gender, place): Promise<string> {
...
//     public async getPlayingTime(subject, time, amount, age, gender, place): Promise<string> {
...
// // Function Example (e.g., generateText.ts)
// import GenerativeAI from "./api"; // Import the GenerativeAI class

// function generateText(prompt: string, genAI: GenerativeAI): Promise<string> {
//     return genAI.generateContent(prompt);
// }

// // Usage
// const someText = await generateText("This is a prompt", genAI);
