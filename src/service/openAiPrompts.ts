    ...
    messages: [
        {
            role: "user",
            content:
                "give me 5 diverse topics in hebrew About cool and intersting topics that can be passed on to children",
        },
    ],
    functions: [
        {
...
export async function getSubjects() {
    const response = await axios(request(subjectsPrompt));
...
export async function getPointOfViewSubject() {
    const response = await axios(request(pointOfViewSubjectPrompt));
    const responseData = response.data;
    ...
    messages: [
        {
            role: "user",
            content:
                "בכל פעם שאני שואל, מצא לי נושא חדש ופשוט שמתאים להעברת פעילות בתנועות נוער ומתאים לדיון עם ילדים בגילאים 8-18. הפעם אני מחפש משהו שלא הצגת לפני כן. תביא לי את הנושא בלי גרשיים. נושא בפחות מ-70 תווים נא להחזיר את התשובה בעברית, תשמור על עברית תקינה ובלי שגיאות כתיב, התשובה צריכה להיות פשוטה ומובנת לילדים.",
        },
    ],
    max_tokens: 70,
    top_p: 1,
...
export async function getContentActivitySubject() {
    const response = await axios(request(contentActivitySubjectPrompt));
    const responseData = response.data;
    const subjectListString = responseData.choices[0].message.content;
    return subjectListString;
}
...
const scoutingSkillSubjectPromptHebrew = {
    ...DefualtPromptDetails(0.5),
    messages: [
        {
            role: "user",
            content:
                "תן לי דוגמא אחת בלבד למיומנויות שלומדים בתנועת נוער צופים, דוגמאות לתשובות רצויות: הדלקת מדורה, הכנת אוהל. תביא לי את התשובה בעברית.",
        },
    ],
    max_tokens: 70,
    top_p: 1,
...
export async function getScoutingTimeSubject() {
    const response = await axios(request(scoutingSkillSubjectPromptHebrew));
    const responseData = response.data;
    const subjectListString = responseData.choices[0].message.content;
    return subjectListString;
}
    ...
    messages: [
        {
            role: "user",
            content:
                "בכל פעם שאני שואל, מצא לי משחק חברה לפעילות לילדים בתנועות נוער. הפעם אני מחפש משהו שלא הצגת לפני כן. תחזיר לי הסבר קצר עד 6 מילים על המשחק. נא להחזיר את התשובה בעברית. תשמור על עברית תקינה ובלי שגיאות כתיב..",
        },
    ],
    max_tokens: 70,
    top_p: 1,
...
export async function getPlayingTimeSubject() {
    const response = await axios(request(playingTimeSubjectPrompt));
    const responseData = response.data;
    const subjectListString = responseData.choices[0].message.content;
    return subjectListString;
}
