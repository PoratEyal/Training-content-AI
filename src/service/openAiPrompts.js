...
const pointOfViewSubjectPrompt = {
    ...
    messages: [
        {
            role: "user",
            content: "בכל פעם שאני שואל, מצא לי נושא חדש ואקטואלי, ייחודי ומעניין, הקשור לישראל או לעולם, המתאים לדיון עם ילדים. הפעם אני מחפש משהו שלא הצגת לפני כן. תביא לי את הנושא בלי גרשיים. נושא בפחות מ-70 תווים נא להחזיר את התשובה בעברית.",
        },
    ],
    temperature: 0.9,
    max_tokens: 70,
...
const contentActivitySubjectPrompt = {
    ...
    messages: [
        {
            role: "user",
            content: "בכל פעם שאני שואל, מצא לי נושא חדש, ייחודי ומעניין, המתאים לדיון עם ילדים. הפעם אני מחפש משהו שלא הצגת לפני כן. תחזיר לי את הנושא בלי גרשיים. נושא בפחות מ-70 תווים. נא להחזיר את התשובה בעברית.",
        },
    ],
    temperature: 0.9,
    max_tokens: 70,
...
const playingTimeSubjectPrompt = {
    ...
    messages: [
        {
            role: "user",
            content: "בכל פעם שאני שואל, מצא לי משחק חברה לפעילות לילדים בתנועות נוער. הפעם אני מחפש משהו שלא הצגת לפני כן. תחזיר לי הסבר קצר עד 70 תווים על המשחק. נא להחזיר את התשובה בעברית.",
        },
    ],
    temperature: 0.9,
    max_tokens: 70,
