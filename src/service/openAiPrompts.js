...
const pointOfViewPrompt = (subject, time) => {
    return {
        model: "gpt-4",
        messages: [{
            role: "user",
            content: `A "point of view activity" is an activity that the instructor does with his mentees, focusing on a certain topical issue. You are a scoutmaster and i want you to create a "Point of View Activity" on the topic of ${subject} that will last ${time}. Return the answer in Hebrew! For example, I will scatter sentences on the floor to help participants understand the situation (if the activity is delivered online, the sentences can be shown on the screen in a presentation): - The glaciers are melting faster and losing 31% more snow and ice per year compared to the situation 15 years ago. - The glaciers in the mountains lose more than 328 billion tons of ice and snow every year. - The water from the melted glaciers flows into the sea, and as a result, the sea level rises. - Between the years 2015 and 2019, the melting rate of the glaciers is 78 billion tons more than the rate between 2000 and 2004. - By the end of the 21st century, the sea level is expected to rise by one meter, turning a billion people into refugees. Questions for discussion: - How does that happen? - Why is it happening? - How can this be prevented? - Why is it important to prevent it?`
        }],
        temperature: 0.7,
    };
};

export async function getPointOfView (subject, time) {
    const requestOptions = {
        method: "post",
        url: OpenAIUrl,
        data: pointOfViewPrompt(subject, time),
        headers: openAiheaders,
    };
...
export async function getContentActivity (subject, time) {
    const requestOptions = {
        method: "post",
        url: OpenAIUrl,
        data: contentActivityPrompt(subject, time),
        headers: openAiheaders,
    };
...
const playingTimePrompt = (subject, time) => {
    return {
        model: "gpt-4",
        messages: [{
            role: "user",
            content: `You are a scoutmaster and i want you to create a "Playing time" activity on the topic of ${subject} that will last ${time}. Return the answer in Hebrew! For example, Every time I tell the trainees a fact about me, and they have to guess whether it is true or false.`
        }],
        temperature: 0.7,
    };
};

export async function getPlayingTime (subject, time) {
    const requestOptions = {
        method: "post",
        url: OpenAIUrl,
        data: playingTimePrompt(subject, time),
        headers: openAiheaders,
    };
