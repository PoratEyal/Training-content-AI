...
export const ContentProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    useEffect(() => {
        if (!data && currentUser && currentUser.movement) {
            const { grade, amount, gender, movement, time } = currentUser.movement;
            setData({
    ...
    const updateDetails = (movement, grade, amount, time, gender) => {
        setData((prevData) => {
            const data = addSessionData(movement, grade, amount, time, gender);
