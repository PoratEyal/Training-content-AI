...
export async function getPointOfView(activityDetials: ActivityDetails): Promise<string> {
    const { subject, time, amount, grade, gender, place } = activityDetials;
    const prompt = `אתה מדריך צופים המעביר פעילות לחניכיו בתנועת הנוער ואני רוצה שתיצור פעילות בשם 'פעילות נקודת מבט' על הנושא ${subject} שתימשך ${time}, מספר הילדים בפעילות הוא ${amount}, כיתתם היא ${grade} ומינם הוא ${gender}. מקום הפעילות יהיה ב${place}. (וודא שאין שגיאות כתיב כלל) בתחילת התשובה כתוב את שם הפעילות ואת משך הזמן שלה. בכל חלק של הפעילות, ציין את הזמן הנדרש בדקות. וודא כיסוי מלא של כל ההיבטים והוראות מפורטות לפעילות. תחזיר את התשובה בפורמט של markdown style (תוודא שאתה לא מוסיף שפות אחרות). הבט לשקף תכנים ישירים כמו שמות וזמנים ללא שינוי, אך העשר את התיאור בפרטים רלוונטיים נוספים להבנה טובה יותר. השתדל לייצר פעילות שבה המדריך פחות מדבר וחניכים יותר פעילים. בנוסף, כלול הסברים מפורטים ומעמיקים על הנושא המרכזי של הפעילות, כך שהמדריך לא יצטרך לחפש מידע נוסף ממקורות אחרים. הסבר את התכנים של הפעילות בפירוט רב, כולל רקע היסטורי, עובדות מעניינות ודוגמאות רלוונטיות. אל תכלול מצגת או וידאו בפעילות. התשובה צריכה להיות מפורטת ובעברית בלבד.`;
    return await generateContent(prompt);
}
